...
class PixelTranslator(Translator):
    ...
    def sample(self, stage, coord_expr, projective=False):
        s = self.sampler(stage)
        if self.is_cube(stage):
            return "texture(%s, (%s).xyz)" % (s, coord_expr)
        if projective:
            return "textureProj(%s, (%s).xyw)" % (s, coord_expr)
        return "texture(%s, (%s).xy)" % (s, coord_expr)
    ...
    def source(self, name, instrs, alpha_test):
        ...
        for n in sorted(self.stages_used):
            samplers += "uniform %s s%d;\n" % ("samplerCube" if self.is_cube(n) else "sampler2D", n)
        discard = ""
        if alpha_test:
            discard = "    if (r0.a < alphaRef) discard;   // D3DRS_ALPHATEST for this shader\n"
        return PS_TEMPLATE % {"name": name, "version": self.version, "samplers": samplers,
                              "temps": temps, "body": body, "discard": discard}
...
{
%(temps)s%(body)s
%(discard)s    fragColor = clamp(r0, 0.0, 1.0);
}
"""
...
def translate_ps(name, cube_mask, alpha_test):
    version, instrs = parse(load_ps_text(name))
    return PixelTranslator(cube_mask, version).source(name, instrs, alpha_test)
...
def main():
    ...
    if args.check:
        ...
        if n.startswith("vs"):
            ...
            info = next(p for p in index["pixel"] if p["name"] == n)
            print(translate_ps(n, 0, info["alpha_test"]))
        return
    ...
    for p in index["pixel"]:
        try:
            src = translate_ps(p["name"], 0, p["alpha_test"])
